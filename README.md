...
**Imágenes:**
- Una imagen guardada como PNG sin tocar conserva la marca perfectamente **si tiene textura real** (fotografías, documentos con contenido variado). ⚠️ **Hallazgo nuevo (2026-08-11), al verificar el fix de recortes**: imágenes con muy poca variación de color (un degradado liso, o un fondo blanco con solo unas pocas líneas negras — el tipo de imagen típico de un documento escaneado simple) pueden fallar al leer la marca **incluso recién marcadas, sin ningún recorte ni compresión de por medio**. La causa es que el suavizado (blur) que se aplica para que la marca no se note a simple vista, combinado con el cambio de tamaño de ida y vuelta al tamaño fijo de trabajo, puede borrar la señal si la imagen no tiene suficiente "textura" en la que esconderla. Probado con fotos reales del usuario (funciona perfecto) y con imágenes sintéticas de bajo detalle (falla) para aislar la causa. **Esto podría afectar al marcado de páginas escaneadas de PDF simples** (texto en blanco y negro sin apenas textura), que usa este mismo código de imágenes — no se ha confirmado todavía con un escaneo real, queda pendiente de investigar si hace falta.
...
- [ ] **Investigar el fallo con imágenes de poca textura** (encontrado el 2026-08-11 al verificar el fix de recortes) — degradados lisos o imágenes tipo "documento escaneado simple" (fondo blanco, pocas líneas negras) pueden perder la marca incluso sin recorte ni compresión. Confirmar si afecta a escaneos reales (no solo a las imágenes sintéticas de prueba) y, si es así, decidir un ajuste (menos suavizado, o repartir la marca de forma distinta cuando hay poca variación de color).
