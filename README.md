...
### Tolerancia a márgenes/recortes en capturas de pantalla

Tras el cambio de tamaño fijo quedaba otro caso frágil: una captura de pantalla que **encuadra el contenido con un margen alrededor** (la barra de herramientas de un visor, el fondo del escritorio, un scroll no perfecto) rompía la lectura por completo, aunque la imagen en sí estuviera intacta — el algoritmo necesita que el encuadre coincida casi al píxel con el que se usó al marcar.

**La solución**: al leer una marca, además de intentarlo con la imagen tal cual llega, se analiza cada fila y columna por separado para detectar si hay un margen de color prácticamente uniforme alrededor del contenido real (una captura de pantalla digital tiene bordes nítidos, a diferencia de una foto de cámara, que los difumina — por eso esta técnica funciona aquí y no la usamos para el caso de fotos a pantalla). Si se detecta un margen, se recorta automáticamente y se intenta leer también esa versión recortada. Cada lectura candidata se contrasta contra los códigos ya emitidos en la base de datos, y se usa la primera que coincida exactamente — así no se corre el riesgo de "inventar" un código que se parezca a uno real por casualidad.

**Probado**: margen asimétrico simulado (distinto grosor en cada lado) sobre una imagen ya marcada — la marca se recuperó exacta tanto con margen como sin él, y también combinado con el caso de WhatsApp (redimensionado + compresión). Un recorte que se come contenido real de la imagen (no solo el margen añadido) sigue sin poder recuperarse, como es esperable — no hay margen que quitar, hay información perdida de verdad.

### Por qué se evita el blanco y negro puros

Al verificar el fix de recortes con una **página escaneada de texto real** (fondo blanco, texto negro, como un documento de oficina), apareció un fallo nuevo y más serio: la marca se perdía **incluso recién marcada, sin recorte ni compresión de por medio**. Con fotos normales (con textura natural) funcionaba perfecto; con esta página de texto, siempre vacío.

**La causa exacta** (se aisló paso a paso, comprobando cada etapa del proceso por separado): la marca se esconde en el canal de color (no en el brillo), precisamente porque el ojo humano apenas nota los cambios de color — es lo que hace que sea invisible. Pero un píxel **blanco puro** (255,255,255) no tiene ningún margen de color que dar: cualquier intento de moverlo ligeramente hacia un tono u otro obliga a algún canal (rojo, verde o azul) a superar 255, y el sistema lo recorta de vuelta a blanco puro en silencio, borrando el cambio que se acababa de hacer. Lo mismo le pasa al negro puro (0,0,0) en la otra dirección. Una fotografía normal casi nunca tiene píxeles en el extremo exacto; un documento escaneado en blanco y negro está lleno de ellos.

**La solución**: antes de marcar, se acerca el blanco y el negro puros al centro en solo 3 unidades sobre 255 (blanco puro pasa a "casi blanco", negro puro a "casi negro") — un cambio de brillo completamente imperceptible a simple vista, pero suficiente para que quede margen donde esconder la marca. Se comprobó visualmente (incluida una imagen de gradiente, el peor caso para notar un patrón) que no se aprecia ninguna diferencia.

De paso, se ajustó tambien el suavizado que se aplica a tamaño completo (ver [Por qué se marca a un tamaño fijo](#por-qué-se-marca-a-un-tamaño-fijo-no-al-tamaño-original-de-la-imagen)): era un poco más fuerte de lo necesario y, en imágenes con muy poco margen para la marca como esta, terminaba de borrar la señal. Se redujo y se confirmó visualmente que sigue sin verse ninguna cuadrícula.

**Probado**: página de texto escaneada simulada (con y sin grano de escáner realista) marcada y leída de vuelta correctamente, tanto como imagen suelta como dentro de un PDF (la misma página, como página escaneada). También se repitió toda la batería de pruebas anteriores (foto real, WhatsApp, captura con margen) para confirmar que este cambio no rompe nada de lo ya conseguido. El caso de compresión agresiva (tipo WhatsApp) sobre una página de texto *sintética y perfectamente lisa* (sin ningún grano, un caso más extremo que cualquier escaneo real) puede quedar a 2-4 caracteres de diferencia — la versión con grano de escáner realista, más parecida a un documento real, sí sobrevive completa.
...
**Imágenes:**
- Una imagen guardada como PNG sin tocar conserva la marca perfectamente, incluyendo documentos escaneados de solo texto (fondo blanco, líneas negras) — ver [Por qué se evita el blanco y negro puros](#por-qué-se-evita-el-blanco-y-negro-puros).
...
- **Dibujos técnicos con líneas muy finas sobre fondo casi todo blanco (planos, CAD)**: si a una imagen así se le hace una captura de pantalla y **esa captura se reenvía de nuevo por WhatsApp como imagen**, la marca se puede perder por completo — confirmado con un caso real (2026-08-11). No es un fallo general: fotos normales y documentos de texto normales (aunque también sean blanco y negro) sobreviven bien a este mismo escenario, probado explícitamente. El problema es específico de este tipo de contenido: casi toda la página es blanco puro sin ninguna línea, dejando muy poco sitio real donde esconder la marca, y dos compresiones seguidas (la captura + el reenvío) agotan ese poco margen. Se probó subir mucho el margen de seguridad del blanco/negro puro y no lo soluciona — el problema no es el mismo que el de blanco/negro puro de [más abajo](#por-qué-se-evita-el-blanco-y-negro-puros), haría falta repartir la marca de otra forma específicamente para este tipo de contenido disperso. Fuera del alcance del proyecto por ahora, igual que el caso de foto-de-cámara-a-pantalla.
...
- Las páginas que ya son un escaneo (solo imagen, sin texto) se marcan con la misma técnica que las imágenes sueltas — hereda su misma robustez ante cambios de resolución, y también el fix de blanco/negro puros (ver [Por qué se evita el blanco y negro puros](#por-qué-se-evita-el-blanco-y-negro-puros)), confirmado con una página de texto escaneada simulada.
- **Reenviar el PDF como documento (WhatsApp, email, etc.)**: **confirmado con un envío real por WhatsApp (2026-08-11)** — el archivo marcado y el recibido por WhatsApp son **idénticos byte a byte** (mismo hash). A diferencia de las imágenes (que WhatsApp siempre recomprime y redimensiona), los PDFs los envía como documento "tal cual", sin tocarlos — así que la marca llega perfecta, sin necesitar ningún margen de tolerancia. También probado que sobrevive a que otra librería reescriba el archivo por completo, y a compresores de PDF típicos usados moderadamente (probado con una reducción del 60% + JPEG calidad 70, quedó casi exacto) — esto cubre el caso de que alguien comprima el PDF a propósito antes de reenviarlo, algo que WhatsApp no hace pero otras herramientas sí podrían. Con compresores muy agresivos (reducir a menos del 40% del tamaño) puede degradarse más allá de lo que cubre la búsqueda aproximada — mismo límite que ya conocíamos para imágenes muy comprimidas.
...
- **Captura de pantalla de una página escaneada**: funciona bien a distintas resoluciones de pantalla (96 a 220 DPI) y en PNG o JPEG. Si la captura incluye un margen alrededor del contenido (barra de herramientas de un visor, fondo, scroll no perfecto), ahora se detecta y se recorta automáticamente antes de leer la marca (ver [Tolerancia a márgenes/recortes en capturas de pantalla](#tolerancia-a-márgenesrecortes-en-capturas-de-pantalla)) — antes esto rompía la lectura por completo con solo un 0.5% de margen. Sigue haciendo falta que el margen sea de un color prácticamente uniforme (como el fondo típico de un visor); un recorte que se coma contenido real de la imagen, o un fondo que no sea uniforme, sigue sin poder recuperarse. Para páginas escaneadas de dibujos técnicos (líneas finas, casi todo blanco) aplica además la limitación de arriba: si esa captura se reenvía otra vez por WhatsApp como imagen, la marca se puede perder — confirmado con un caso real de un plano arquitectónico enviado como PDF, capturado en el móvil y reenviado.
...
- [x] **Investigar si los PDFs sobreviven a reenvío y a capturas de pantalla** (2026-08-11) — probado y documentado en detalle en [Limitaciones conocidas](#limitaciones-conocidas-probadas-no-teóricas): el reenvío como documento sobrevive bien, **confirmado con un envío real por WhatsApp** (el archivo llega idéntico byte a byte, WhatsApp no lo recomprime como hace con las imágenes); las capturas de pantalla de páginas con texto nunca sobreviven (esperable); las capturas de páginas escaneadas sobreviven, incluso con margen alrededor, gracias al fix de recortes y al de blanco/negro puros.
