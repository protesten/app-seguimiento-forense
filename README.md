...
- Recomprimida como JPEG de buena calidad (90-95) y en resolución alta (ej. 1920x1080), la marca sobrevive bien.
- Con compresión muy agresiva (calidad ≤70) puede aparecer **algún carácter incorrecto** en el código recuperado.
- Una **foto tomada con el móvil a la pantalla** (en vez de una captura digital) **no conserva la marca — confirmado tanto en una prueba real (usuario: foto + reenvío por WhatsApp) como en simulaciones controladas**. Investigamos si una corrección automática de perspectiva podía rescatarla: se descartó porque `dwtDctSvd` solo tolera ~3 píxeles de error de alineación en una imagen de 1920px (0.15%), una precisión que ninguna detección de bordes sobre una foto real puede alcanzar (el propio borde físico ya sale borroso por el enfoque de la cámara). No es un problema de "afinar el algoritmo" — haría falta una técnica de marcado totalmente distinta (invariante a rotación/escala), que queda fuera del alcance actual del proyecto.
...
- Si te preocupan los casos límite de compresión agresiva (no de foto-de-pantalla), una mejora futura es comparar el código detectado contra la lista de códigos reales que emitiste y quedarte con el más parecido (tolera 1-2 caracteres de error), en vez de exigir coincidencia exacta.
